...
function block_slider_pluginfile($course, $birecord_or_cm, $context, $filearea, $args, $forcedownload, array $options=array()) {
    ...
    if (!$file = $fs->get_file($context->id, 'block_slider', 'slidecontent', $itemid, $filepath, $filename) or $file->is_directory()) {
        send_file_not_found();
    }
 ...
 * @return void
 */
function block_slider_global_db_replace($search, $replace) {
    global $DB;

    $instances = $DB->get_recordset('block_instances', array('blockname' => 'slider'));
